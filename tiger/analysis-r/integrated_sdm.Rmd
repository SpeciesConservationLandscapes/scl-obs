...
## AD HOC, SITE OCCUPANCY, AND PRESENCE-ABSENCE DATA 
----------------------------------------------------------------------------
...
# spatial points
# should be a list of lat long values
...
Data for Indonesia downloaded from https://www.diva-gis.org/datadown as an exercise. HII and other covariates will be included in the final model.
...
# using csvs from Koshkina paper for now
so.occupancy = read.csv("so_occupancy.csv")

# This need to be csvs or a matrix of variables and their values across the survey area that affect detection
# using csvs from Koshkina paper for now
so.detection = read.csv("so_detection.csv")
...
#turning rasters into matrix - background, adding column of ones
...
Uses a function that fits IPP model.
