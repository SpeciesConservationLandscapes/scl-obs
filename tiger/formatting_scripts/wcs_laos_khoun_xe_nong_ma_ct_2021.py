...
data = pd.read_csv(os.path.join(UNFORMATTED_DIR, "CT_ ICBF_WCS_Data Compilation_KXNM_2021.csv"))
...
ct_deployment["deployment ID"] = data["Grids"].map(str) + "_" + data["Camera1"].map(str)
...
ct_deployment["pickup date/time"] = pd.to_datetime(data["Date retrieve"] + ' ' + data["Time retrieve"].map(str), errors="coerce")
...
ct_deployment_csv = os.path.join(FORMATTED_DIR, "wcs_2021_laos_khoun_xe_nong_ma_ct_deployments_latlon.csv")
