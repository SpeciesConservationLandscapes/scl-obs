...
data = pd.read_excel(os.path.join(UNFORMATTED_DIR, "CT_NEPL_WCS_Data Compilation.xlsx"), sheet_name="Tiger_CT-Loc_2013-2014")
...
data["deployment ID"] = data["Location_ID"].map(str) + "_" + data["CT Number"].map(str)
data.loc[data['Scientific_name'] == 'Panthera tigris', "# adults sex unknown" ] = 1
