...
# ct_deployment.loc[:, 'country'] = 'Myanmar'
# ct_deployment.loc[:, 'name of observer/PI'] = 'Hla Naing'
# ct_deployment.loc[:, 'organizational affiliation'] = 'WCS'
# ct_deployment.loc[:, 'email address'] = 'hnaing@wcs.org'
...
ct_deployment['deployment date/time'] = pd.to_datetime(data['DateSet'])
...
ct_deployment_csv = os.path.join(FORMATTED_DIR,
                                 'CamComAll_30min MASTER_Hukaung_ct_deployment_formatted.csv')
ct_deployment.to_csv(ct_deployment_csv, index=False)
#
ct_observation_csv = os.path.join(FORMATTED_DIR,
                                 'CamComAll_30min MASTER_Hukaung_ct_observation_formatted.csv')
